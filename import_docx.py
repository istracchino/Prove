...
def selProd(recPath):
    ...
    ii=1
    for x in os.scandir(os.path.join(fam[sel],'Wv/Ricette')):
        if x.is_dir() == True:
    ...
    return os.path.join(prod[sel2][0]+'/'+str(sel3)+'Op'),prod[sel2][1],os.path.join(fam[sel],'Wv/image')
def listZIP():
